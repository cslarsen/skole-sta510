...
\]
\paragraph{$\boldsymbol{P(X \leqslant 0.3)}$} is found by using $F(0.3)$.
The reason $F$ gives us the probability of $X$ being less than \textit{or
equal to} $x$ is because $P(x) = 0$ for continuous distributions. We get
\[
  P(X \leqslant 0.3) = F(0.3) = 2\cdot0.3^2 - 0.3^4 = 0.18 - 0.0081 = 0.1719
\]
