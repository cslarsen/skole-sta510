...
\[
  \Prb(X\leqslant y)^m = (1 - e^{-\frac{y^2}{2\theta^2}})^m = \Prb(Y \leqslant y)
\]
because each wave is independent of another (which is most likely not true in
real life).

Now, the reverse would be that \textit{at least one} wave is higher than $y$.
We can express that as
\[
  \Prb(Y > y) = 1 - \Prb(X\leqslant y)^m = 1 - (1 - e^{-\frac{y^2}{2\theta^2}})^m
\]
$\hfill\blacksquare$
