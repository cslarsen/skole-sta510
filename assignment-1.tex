...
\[
  \Prb(X\leqslant y) = 1 - e^{-\frac{x^2}{2\theta^2}}
\]
The chance that all $m$ waves are smaller than or equal to $y$ will then be
\[
  \Prb(X\leqslant y)^m = (1 - e^{-\frac{x^2}{2\theta^2}})^m
\]
This uses the fact that each wave is independent of the other (which is most
likely not true in real life). It's just like the chance of two coin tosses
being heads, i.e.~$\frac{1}{2}\cdot\frac{1}{2} = \frac{1}{4}$.

Now, the reverse would be that \textit{at least one} wave is higher than $y$,
expressed as
\[
  \Prb(X>y) = 1 - \Prb(X\leqslant y)^m = 1 - (1 - e^{-\frac{x^2}{2\theta^2}})^m
  \hfill\blacksquare
\]
