...
\problem{3 (e)}
With acceptance-rejection sampling, we are supposed to assume that the PDF of the triangle distribution, or that it
is impractical to sample from directly. Instead, we should find a function with
similar shape and find a constant $c$ so that $\frac{f(t)}{cg(t)} \leqslant c$
for all $t$. But since we need a function that follows the shape of the
triangular PDF itself, we must reimplement it but with a factor off
so that we can use acceptance-rejection sampling. Instead, we just set
$c=\frac{2}{(b-a)}$ to normalize $f$ so that we check if $u < \sfrac{f(y)}{c}$.
