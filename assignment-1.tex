...
We then perform a large number of simulations and divide the sum of each
simulation by the number of simulations made.
...
throw away the numbers if they are less than this comparison. He also shows how
his algorithm enables to embed simpler mathematical expressions directly in the
code, if applicable.
...
In \cite{STEIN20091143} we see that the triangle distribution does not have a
closed form, and therefore cannot be used with the inverse transform method.
The paper presents multiple alternatives, such as the MINIMAX-method, but also
presents a novel method of calculating the triangle PDF in one line. However, I
have used that formula in my code, because I wanted to mimic the code given in
the lectures.

It also seems that in papers like \cite{stanford} the comparison is
\textit{less-than-or-equal} instead of just \textit{less-than}. I have also
used that in my code, because I found it reasonable --- even though most of the
code I've seen does use the less-than binary operator.
